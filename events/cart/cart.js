            ...
            await interaction.guild.channels.create({
                ...
                await channel.send({
                    ...
                    embeds:[
                        ...
                        .setFooter({text:"Copyright - yInvicto"})
                        .addFields(
                            {
                ...
                interaction.editReply({
                    content:`${emoji.carrin} | O seu carrinho foi aberto com sucesso em: ${channel}`, 
                    components:[
                    new ActionRowBuilder()
                    .addComponents(
                        new ButtonBuilder()
...
const checkPaymentStatus = setInterval(async () => {
  try {
    const token = await bot.get("acesstoken");
    ...
    const statusCart = await cart.get(`${interaction.channel.id}.status`);

    if (statusCart === "pendente" && payment.status === "approved") {
    ...
    // Delay normal do Mercado Pago — IGNORA
             ...
             interaction.editReply({ content: ``, embeds: [embed], components: [row], ephemeral: true }).then(msg => {
                 ...
                 collector.on('collect', interaction2 => {
                     ...
                     if (interaction2.customId == 'verificarpagamento') {
                        try {
                            axios.get(`https://api.mercadopago.com/v1/payments/${data.body.id}`, {
                            headers: {
