...
module.exports = {
    ...
    run:async(interaction, client) => {
        ...
        if(customId.endsWith("_titlepainel")) {
            ...
            .setStyle(1)
            .setRequired(true)
            .setPlaceholder("Coloca essa bosta logo, seu banana");
        ...
        if(customId.endsWith("_bannerpainel")) {
            ...
            .setCustomId("text")
            .setStyle(1)
            .setPlaceholder('ei idiota, digita "remover" para tirar')
        ...
        async function configedit() {
            ...
            const p = await db2.get(`${id}`);
        if(!p) return interaction.reply({content:`Não existe nenhum painel com este ID.`, ephemeral:true});
        let banner = "`Sem Banner`"
        if(p.banner?.startsWith("https://")) {
            banner = `[Banner](${p.banner})`
        }
